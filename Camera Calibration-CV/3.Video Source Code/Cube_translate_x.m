...
v = VideoWriter('Cube_translate_x.avi'); %output vid file name
v.FrameRate = 30; %setting fps
open(v); %opening to begin writing

for i = 0:0.1:9
    
    [cube_1_x,cube_1_y] = cameracalib(0+i,0,0);
[cube_2_x,cube_2_y] = cameracalib(1+i,0,0);
[cube_3_x,cube_3_y] = cameracalib(0+i,1,0);
[cube_4_x,cube_4_y] = cameracalib(1+i,1,0);
[cube_5_x,cube_5_y] = cameracalib(0+i,0,1);
[cube_6_x,cube_6_y] = cameracalib(1+i,0,1);
[cube_7_x,cube_7_y] = cameracalib(0+i,1,1);
[cube_8_x,cube_8_y] = cameracalib(1+i,1,1);

%hardcoding the cube coordinates
...
%my method involves the saving of figure to an image, storing it onto the
%disk, reading it and then writing it to the movie
writeVideo(v,G);
